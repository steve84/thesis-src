...
pure_titles = loadInputs('../../../../pure/iteration_1/tasks/evaluation/input_title_evaluation.csv')
pure_descriptions = loadInputs('../../../../pure/iteration_1/tasks/evaluation/input_description_evaluation.csv')
pure_categories = loadInputs('../../../../pure/iteration_1/tasks/evaluation/input_category_evaluation.csv')
...
with open(path_groundtruth, 'rb') as csvfile:
    ...
    next(reader)
    if ((length == len(titles)) & (length == len(descriptions)) & (length == len(categories)) & (length == len(pure_titles)) & (length == len(pure_descriptions)) & (length == len(pure_categories))):     
        for row in reader:
            #---------------  BUILD OVERVIEW -------------------
            pure_commission_content = [str(titles[count-1]), str(descriptions[count-1]), str(categories[count-1])]
            pure_content = [str(pure_titles[count-1]), str(pure_descriptions[count-1]), str(pure_categories[count-1])]          
            options = [pure_commission_content, pure_content]
