...
import json, csv, unicodedata, dateutil.parser, re
...
def createCsvFile(file_name, itemList):
    with open(file_name, 'wb') as test_file:
        ...
        file_writer.writerow(['Title','Description','Category','ID','Item ID','Condition','Price','Model','Image 1','Image 2','Image 3'])
        for item in itemList:
            file_writer.writerow([str(item[0]), item[1], str(item[2]), str(item[3]), str(item[4]), str(item[5]), str(item[6]), str(item[7]), str(item[22][0]), str(item[22][1]), str(item[22][2])])
...
def createCsvTrainingFile(file_name, dataList, labelList, label=False, val_range=5, normalize=False):
    with open(file_name, 'wb') as test_file:
        file_writer = csv.writer(test_file, csv.excel_tab)
        file_writer.writerow(['Condition','Model','Seller rating','Seller rating count','Duration','Number of pictures','Length of description','End Weekday','Start Weekday','End Hour','Handling time','Global shipping','Shipping Locations','Shipping Type','Return accepted','Price'])
        ...
        if not normalize:
            for x in range(0,len(dataList)):
                ...
                file_writer.writerow([str(dataList[x][0]), str(dataList[x][1]), str(dataList[x][3]), str(dataList[x][4]), str(dataList[x][5]), str(dataList[x][6]), str(dataList[x][7]), str(dataList[x][8]), str(dataList[x][9]), str(dataList[x][10]), str(dataList[x][11]), str(dataList[x][12]), str(dataList[x][13]), str(dataList[x][14]), str(price_label)])
        else:
            # create min/max values for every feature
            minMaxValues = list()
            minMaxValues.append([1, 2]) #0 Condition
            minMaxValues.append([1964, 2014]) #1 Model
            minMaxValues.append([0, 100]) #2 Seller rating
            minMaxValues.append([0, 12]) #3 Seller rating count
            minMaxValues.append([1, 10]) #4 Duration
            minMaxValues.append([1, 12]) #5 Number of pictures
            minMaxValues.append([0, 500000]) #6 Length of description
            minMaxValues.append([1, 7]) #7 End wekkday
            minMaxValues.append([1, 7]) #8 Start weekday
            minMaxValues.append([0, 23]) #9 End hour
            minMaxValues.append([0, 20]) #10 Handling time
            minMaxValues.append([0, 1]) #11 Global shipping
            minMaxValues.append([0, 249]) #12 Shipping locations
            minMaxValues.append([0, 7]) #13 Shipping type
            minMaxValues.append([0, 1]) #14 Returns accepted
...
def convertModelToNumber(title):
    modelnbr = -1
    # try to find the model number of the car
    if (title.lower().find('mustang') >= 0):
				# e.g. '1978
        found = re.findall("'\d\d\d\d", title)
        if(len(found) == 1):
            modelnbr = int(found[0][1:])
        elif(len(found) == 0):
						# e.g. '78
            found = re.findall("'\d\d", title)
            if(len(found) == 1):
                modelnbr = int(found[0][1:])
                if modelnbr > 14:
                    modelnbr = 1900 + modelnbr
                else:
                    modelnbr = 2000 + modelnbr
            elif(len(found) == 0):
								# if two years are available then take the older one
                found = re.findall("\d\d\d\d", title)
                if(len(found) == 2):
                    if not(found[0] == found[1]):
                        model_1 = int(found[0])
                        model_2 = int(found[1])
                        if model_1 >= model_2:
                            modelnbr = model_2
...
try:
    ...
    keyword = 'hot wheels ford mustang'
    categoryId = '223'
    ...
    createCsvTrainingFile('train/trainDataLabeled.csv', data_train, label_train, label=True, val_range=2)
    createCsvTrainingFile('test/testDataLabeled.csv', data_test, label_test, label=True, val_range=2)
    ...
    createCsvTrainingFile('train/trainDataLabeledNormalized.csv', data_train, label_train, label=True, val_range=2, normalize=True)
    createCsvTrainingFile('test/testDataLabeledNormalized.csv', data_test, label_test, label=True, val_range=2, normalize=True)
