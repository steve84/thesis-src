    ...
    if fields.issubset(response['Item'].keys()) & pure_auction:
        ...
        if len(specifics) > 3:
            for entry in specifics:
                ...
                if (entry['Name']['value'] == 'Storage Capacity'):
                    storage = convertStorageToNumber(entry['Value']['value'], title)
        ...
        if (len(description) > 0) & (len(pictures) > 2) & (condition > 0) & (price != 0) & (storage > 0) & (model >= 0) & (duration > 0) & (seller_rating_count >= 0) & (quantity == 1):
            pictures_list = list()
...
def createCsvFile(file_name, itemList):
    with open(file_name, 'wb') as test_file:
        ...
        file_writer.writerow(['Title','Description','Category','ID','Item ID','Condition','Price','Model','Storage','Image 1','Image 2','Image 3'])
        for item in itemList:
...
def createCsvTrainingFile(file_name, dataList, labelList, label=False, val_range=5, normalize=False):
    with open(file_name, 'wb') as test_file:
        file_writer = csv.writer(test_file, csv.excel_tab)
        file_writer.writerow(['Condition','Model','Storage','Seller rating','Seller rating count','Duration','Number of pictures','Length of description','End Weekday','Start Weekday','End Hour','Handling time','Global shipping','Shipping Locations','Shipping Type','Return accepted','Price'])
        ...
        if not normalize:
            ...
            minMaxValues.append([1, 4]) #2 Storage
...
def convertModelToNumber(model, title):
    ...
    model = model.lower()
    title = title.lower()
    if ((model.find('5s') >= 0) & (title.find('5s') >= 0)):
        modelnbr = 7
    elif ((model.find('5c') >= 0) & (title.find('5c') >= 0)):
        modelnbr = 6
    elif ((model.find('5') >= 0) & (title.find('5') >= 0)):
        modelnbr = 5
    elif ((model.find('4s') >= 0) & (title.find('4s') >= 0)):
        modelnbr = 4
    elif ((model.find('4') >= 0) & (title.find('4') >= 0)):
        modelnbr = 3
    elif ((model.find('3gs') >= 0) & (title.find('3gs') >= 0)):        
        modelnbr = 2
    elif ((model.find('3g') >= 0) & (title.find('3g') >= 0)):        
        modelnbr = 1        
    elif (title.find('1st') >= 0):   
        modelnbr = 0
...
def convertStorageToNumber(storage, title):
    storagenbr = 0
    if ((storage.find('8') >= 0) & (title.find('8') >= 0)):
        storagenbr = 1
    elif ((storage.find('16') >= 0) & (title.find('16') >= 0)):
        storagenbr = 2
    elif ((storage.find('32') >= 0) & (title.find('32') >= 0)):
        storagenbr = 3
    elif ((storage.find('64') >= 0) & (title.find('64') >= 0)):
        storagenbr = 4
        
    return int(storagenbr)
