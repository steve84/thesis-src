...
def loadInputs(csv_file_name):
    with open(csv_file_name, 'rb') as csv_file:
        ...
        for row in file_reader:
            inputs.append(str(row[1]).replace('&','&amp;'))
        return inputs    
...
workerTable = createWorkerTable('workerIds.csv', 5)

titles = loadInputs('input_title_price.csv')
descriptions = loadInputs('input_description_price.csv')
retail_prices = loadInputs('input_retail_prices.csv')
...
with open(path_groundtruth, 'rb') as csvfile:
    reader = csv.reader(csvfile, csv.excel_tab)
    next(reader)
    if (length == len(titles)) & (length == len(descriptions)) & (length == len(retail_prices)):     
        for row in reader: 
            #---------------  BUILD OVERVIEW -------------------
            ...
            html_code += '<h3><b>Retail/Market price</b></h3>'            
            html_code += str(retail_prices[count-1])            
            ...
            html_code += '<li>Estimate the total sales price of the item(s) in USD as positive floating point number with two digits after the point (for example 50.99 or 10.00). <b>Hint:</b> Take a look at the retail price of the item(s) (Source: pricegrabber.com)</li>'
