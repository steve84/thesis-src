...
def createCsvFile(file_name, resultList):
    with open(file_name, 'wb') as test_file:
        ...
        file_writer.writerow(['Ground truth number','Evaluation','Votes of crowdsourced content (in %)','Average working time (s)','Average finishing time (s)','Average reaction time (s)'])
...
def createDatFile(file_name, resultList):
    with open(file_name, 'wb') as test_file:
        ...
        file_writer.writerow(['#Ground truth number','Votes of crowdsourced content (in %)','Average working time (s)','Average finishing time (s)','Average reaction time (s)'])
        for result in resultList:
            if len(result) == 5:
...
def majorityVoting(votes, crowdsourced_option):
    ...
    if vote_list[0][0] >= majority:
        if str(vote_list[0][1]) == str(crowdsourced_option):
            return ['Crowdsourced content',float(float(vote_list[0][0]) / len(votes))]
        else:
            if len(vote_list) > 1:
...
createCsvFile('../../output/output_evaluation.csv', evaluationOutput)
createDatFile('../../output/dat/output_evaluation.dat', evaluationOutput)
